import { deriveKey, encryptData, decryptData } from '../security/security-utils';
...
describe('Crypto Utilities', () => {
    ...
    it("should derive functionally equivalent keys with the same pin and salt", async () => {
        ...
        expect(new TextDecoder().decode(decryptedData)).toEqual("verify key equivalence");
    });
    ...
    it('should fail to decrypt with an incorrect pin', async () => {
        ...
        await expect(decryptData(incorrectPin, salt, iv, ciphertext)).rejects.toThrow();
    });
    ...
    it('should fail to decrypt with an incorrect salt', async () => {
        ...
        await expect(decryptData(pin, incorrectSalt, iv, ciphertext)).rejects.toThrow();
    });
    ...
    it('should fail to decrypt with an incorrect IV', async () => {
        ...
        await expect(decryptData(pin, salt, incorrectIv, ciphertext)).rejects.toThrow();
    });
});